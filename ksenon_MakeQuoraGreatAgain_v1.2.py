...
X_train.to_csv('X_train_quora.csv')
X_test.to_csv('X_test_quora.csv')
...
# There is likely a much more elegant way to do this...
 ...
 train_data_fix = pd.concat([pos_train, neg_train])
...
# 2. Val = 0.1987; Train = 0.1918; LB = 0.1669;
...
### StackNet solution for Quora ###
    ...
    train_mix = (train_data_fix['question1']+ " " +  train_data_fix['question2']).astype(str).values 
    
    test_mix = (test_data['question1']+ " " +  test_data['question2'] ).astype(str).values  
    
    
        #convert to csr
    X_stack=csr_matrix(X_train_fix)

    X_test_stack=csr_matrix(X_test_fin)
    # the tfidf object
    ...
    #retrieve target
    y = train_data_fix['is_duplicate'].values  
